---
term: Wormroot Depths
target_ko: 벌레뿌리 구렁
target_de:
target_ja:
aliases: [Wormroot Depths, 벌레뿌리 구렁]
category: 던전
status: 확정
source_sheet: 던전
source_row: 81
source_file: "raw/ESO (고유)명사 번역 통일안.xlsx"
---

# Wormroot Depths (벌레뿌리 구렁)

## 채택 근거 (UESP 자동 보강)
They are led by a dremora named Raynia.

This delve can be discovered by reading a note at Nazura's Nibbles in the Redfur Trading Post.

*
*
*
*

:Explore and clear Wormroot Depths.
To clear this dungeon and mark it as complete, you need to kill the following bosses:
:

*

There is one achievement associated with this location:

**출처**: `raw/Online/Wormroot Depths.md` (UESP Online dump) — `feature/remains` #D 자동 보강 (2026-05-27)

## 출처
- `raw/ESO (고유)명사 번역 통일안.xlsx` — 시트 *던전*, 행 81
- 한국 ESO 커뮤니티 합의
