---
term: Blessed Crucible
target_ko: 축복받은 시련
target_de:
target_ja:
aliases: [Blessed Crucible, 축복받은 시련]
category: 던전
status: 확정
source_sheet: 던전
source_row: 33
source_file: "raw/ESO (고유)명사 번역 통일안.xlsx"
---

# Blessed Crucible (축복받은 시련)

## 채택 근거 (UESP 자동 보강)
This dungeon takes place in a vast daedric ruin repurposed as a gladiatorial arena. In it, you must help Talres Voren pay off his debts by winning the tournament and becoming the Champion of the Crucible. You can learn about its location from Vlesyl the Skin-Stealer in Rawl'kha, Sluz the Beryl-Chested in Evermore, or Kailstig the Axe in Riften.

**출처**: `raw/Online/Blessed Crucible.md` (UESP Online dump) — `feature/remains` #D 자동 보강 (2026-05-27)

## 출처
- `raw/ESO (고유)명사 번역 통일안.xlsx` — 시트 *던전*, 행 33
- 한국 ESO 커뮤니티 합의
