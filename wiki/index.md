...
총 **630 페이지** (`status:retired` 4 포함). 카테고리별 분포:

| 카테고리 | 수 | 비고 |
|---|---|---|
| [던전](termbase/) | 211 | 시드: 던전 시트 (충돌 6건 통합) |
| [지명](termbase/) | 174 | 시드: 지명사전 (충돌 1건 통합) |
| [게임용어](termbase/) | 93 | 시드: 확정됨 39 + 기타사전 36 + 글리프 16 + 슬래시 분리 2(kinlady/kinlord) |
| [인물](termbase/) | 61 | 시드: 인명사전 |
| [아이템](termbase/) | 63 | 시드: 아이템 + 슬래시 분리 2(jerkin/robe) |
...
`status:retired` 4 페이지: `sip-of-tincture-of-…` (물약 옵션B) + `jerkin--robe` / `kinlady--kinload` / `kinlord--kinlady` (슬래시 분리 색인).

**채택 근거 상태**:
- **168 페이지**: 디씨 자료에 description이 있어 채택 근거 본문 풍부 (예: [[termbase/soul-gem]])
- **371 페이지**: UESP 자동 보강 완료 (raw/Online 316 + raw/Lore 55, `feature/remains` #D 자동). 예: [[termbase/dremora]]
- **72 페이지 (남은 TODO)**: 정확 일치 lore 파일 없음(69) + stub(3). 후속 lint 또는 fix-as-you-go.
...
- `flags: [retired, slash-slug-split]` — 3 페이지(`jerkin--robe`, `kinlady--kinload`, `kinlord--kinlady`). 옵션 B로 분리 후 retire 색인 ([[decisions/seed-import]] 마찰점 #11 해결).
...
- `wiki/lore/` — **33 페이지** (goal ingest 라운드 1, 2026-05-28 기준):
  - 시리즈 색인: `2920-시리즈-색인`, `16-accords-of-madness-시리즈-색인`
  - 2920 시리즈 12권 (Morning Star ~ Evening Star): Tribunal + Reman III + Vivec 역사 fiction
  - 16 Accords 3권 (Hircine, Vaermina, Malacath의 이야기)
  - 36 Lessons of Vivec Sermon 1, 10-18 (Vivec 종교 텍스트, Michael Kirkbride)
  - 기타: `1-스라시안-역병`, `101-uses-for-troll-fat`, `2-baron-admiral-bendu-olo`, `3-all-flags-navy`, `2e-582`, `2nd-manifesto-of-kinlord-rilis-xii`
- raw/Lore 처리율: 105/14101 (0.7%) — 본인 [[lore/log|log]] 참조.
...
2. ~~451 TODO 마커 페이지 채택 근거 보강~~ **🟡 대부분 완료 (feature/remains #D)**: 371/443 UESP 자동 보강. 남은 72개는 lore 매칭 없음(직책/일반 아이템 등) — 번역 사이클 중 fix-as-you-go.
...
7. ~~슬래시 슬러그 정리~~ **✅ 완료 (feature/remains #7)**: 4 신설(jerkin/robe/kinlady/kinlord) + 3 retire 색인. `kinload` 오타는 `kinlady` alias로 보존.
