...
- ❌ **카테고리 단위 사전 일괄 채움 금지** (예: "Daedric Princes 16 전체", "Aedra 8 전체"). 그건 §4-b *시드 import*에 해당하므로 *seed-policy 절차* (§11 권한 트리거)를 따른다. *자연 trigger 없이 사전에 만들어 두는 termbase는 in-link 0으로 *그래프 오염**.
- ⚠ **모호한 의견 해석 금지**: 사용자의 *"이런 거 추가하면 좋을 것 같아요"* / *"그냥 의견이에요"* 같은 발언은 **결정이 *아님*. *명시 권한 트리거*(§11)나 *AskUserQuestion 확인* 거쳐서 진행한다. *의견을 정책으로 해석한 실수는 *재발 방지 1순위*.
